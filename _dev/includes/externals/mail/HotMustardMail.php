...
class htmlMimeMail5
{
    ...
    */
    public function __construct()
    {
        ...
        */
        ...
        $this->headers['X-Mailer'] = 'HotMustardMail <http://www.hotmustardmedia.co.uk/>';
    }
    ...
    */
	...
	/***************************************************************
	These 2 functions allow external files to be read in as the 
	contents for the html / plain text
	**/
	/**
	  * set a detail for the custom template
	  * @key: string; the name of the variable
	  * @value: string; the actual value to set in the template
	**/
	public function setTemplateDetail($key, $value) {
		eval('$this->template'.$key.' = $value;');
	}
	
	/**
	  * return the contents of a file to passed to $this->setText(); or $this->setHTML
	**/
	public function readFile($file) {
		if(file_exists($file)) {
			$rawStr = addslashes(file_get_contents($file));
			eval("\$text = \"$rawStr\";");
			
			$text = stripslashes($text); // added - dh
			$text = utf8_decode($text); // added - dh
			return $text;
		}
	}
	/*****************************************************************/
    ...
    */
	...
	/**
	  * wrapper to add a file as an attachment
	**/
	public function addEmbeddedAttachment($file) {
		$type = mime_content_type($file);
		$this->addAttachment(new fileAttachment($file, $type, new Base64Encoding()));
	}
